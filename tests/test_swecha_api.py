...
def main():
    ...
    print(
        f"🔐 Authentication Required: {status.get('authentication_required', 'Unknown')}"
    )
    print(
        f"📤 Contribution Supported: {status.get('contribution_supported', 'Unknown')}"
    )
    ...
    print(
        f"  📤 Test Contribution Result: {'✅ Success' if test_success else '❌ Failed (Expected)'}"
    )
    ...
    print(
        f"  • Ready for contributions: {'Yes' if status.get('contribution_supported') else 'Not yet'}"
    )
    if status.get("authentication_required"):
        print(
            "  • 🔑 Bearer token required (set SWECHA_API_TOKEN environment variable)"
        )
