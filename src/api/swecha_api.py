...
class SwechaAPI:
    ...
    def __init__(
        self,
        base_url: str = "https://api.corpus.swecha.org",
    ...
    def discover_endpoints(self) -> Dict[str, Any]:
        ...
        for endpoint in potential_endpoints:
            try:
                ...
                if any(
                    keyword in endpoint
                    for keyword in ["contribute", "upload", "submit", "text", "audio"]
                ):
                    test_payload = {"text": "test", "language": "te"}
                    post_response = self.session.post(
                        f"{self.base_url}{endpoint}", json=test_payload, timeout=5
                    )
    ...
    def contribute_text_data(
        self, text: str, language: str = "te", metadata: Optional[Dict] = None
    ) -> bool:
        """
        ...
        if not capabilities["contribution_supported"]:
            self.logger.warning(
                "Text contribution not supported by current API version"
            )
            return False

        try:
            payload = {"text": text, "language": language, "metadata": metadata or {}}
            ...
            for endpoint in endpoints_to_try:
                if endpoint in capabilities["available_endpoints"]:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        self.logger.error(
                            "Authentication required for text contribution"
                        )
                        return False
                        ...
                        self.logger.warning(
                            f"Text contribution failed via {endpoint}: {response.status_code}"
                        )
    ...
    def contribute_audio_transcription(
        self,
        audio_path: str,
        transcription: str,
        language: str = "te",
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.logger.warning(
                    f"Audio-transcription contribution failed: {response.status_code}"
                )
                return False
    ...
    def search_corpus(
        self, query: str, language: str = "te", limit: int = 10
    ) -> List[Dict]:
        """
...
class WhispNoteSwechaIntegration:
        ...
        try:
            if self.api.health_check():
                ...
                if capabilities["authentication_required"]:
                    self.logger.warning(
                        "Swecha API requires authentication - bearer token needed"
                    )
                if not capabilities["contribution_supported"]:
                    self.logger.info(
                        "Swecha API contribution endpoints not yet available"
                    )
                else:
    ...
    def contribute_whispnote_data(
        ...
        if not capabilities["contribution_supported"]:
            self.logger.info(
                "Swecha API contribution endpoints not yet available - storing locally for future upload"
            )
        ...
        if audio_path and Path(audio_path).exists():
            if self.api.contribute_audio_transcription(
                audio_path, transcription, "te", metadata
            ):
                success = True
                self.logger.info(
                    "Successfully contributed audio-transcription to Swecha corpus"
                )

        return success

    def _store_for_future_upload(
        self, transcription: str, audio_path: Optional[str], language_code: str
    ):
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication_required": capabilities.get(
                "authentication_required", False
            ),
