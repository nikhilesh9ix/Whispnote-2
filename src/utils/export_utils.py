...
class ExportUtils:
    ...
    def create_export_archive(
        self, notes: List[Dict], formats: List[str] = ["markdown", "json"]
    ) -> bytes:
        """
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for i, note in enumerate(notes):
                    ...
                    for fmt in formats:
                        try:
                            ...
                            st.warning(
                                f"Failed to export note {note_id} as {fmt}: {str(e)}"
                            )
                ...
                for fmt in formats:
                    try:
                        ...
                        with open(
                            temp_path / combined_filename, "w", encoding="utf-8"
                        ) as f:
                ...
                with zipfile.ZipFile(
                    zip_buffer.name, "w", zipfile.ZIP_DEFLATED
                ) as zip_file:
    ...
    def _export_docx(self, note_data: Dict) -> str:
        ...
        try:
            ...
            metadata_para.add_run(
                f"Date: {note_data.get('timestamp', 'Unknown')}\n"
            ).bold = True
    ...
    def _export_pdf(self, note_data: Dict) -> str:
        ...
        try:
            ...
            if note_data.get("audio_file"):
                metadata_text += (
                    f"<b>Audio File:</b> {note_data.get('audio_file')}<br/>"
                )
            ...
            if note_data.get("transcription"):
                ...
                story.append(
                    Paragraph(note_data.get("transcription"), styles["Normal"])
                )
                story.append(Spacer(1, 12))
            ...
            if note_data.get("tags"):
                ...
                tags = note_data.get("tags", [])
                story.append(
                    Paragraph(" ".join([f"#{tag}" for tag in tags]), styles["Normal"])
                )
    ...
    def _export_multiple_markdown(self, notes: List[Dict]) -> str:
        ...
        try:
            ...
            for i, note in enumerate(notes, 1):
                ...
                md_content.append(
                    f"{i}. [{title}](#{i}-{title.lower().replace(' ', '-').replace('/', '-')})"
                )
            md_content.append("")
